...
static inline bool pmcr_set_check(unsigned x) {
	if ((x & PMCR_WRITE) == x) {
		pmcr_set(x);
		return 1;
	}
...
static inline void pmccntr_enable(bool cycles_64_bit, bool count_every_64) {
	pmcr_enable_pmccntr(cycles_64_bit, count_every_64);
	cntens_enable_pmccntr();
}

static inline void pmccntr_enable_user(bool cycles_64_bit, bool count_every_64) {
